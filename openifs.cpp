...
int main(int argc, char** argv) {
    ...
    // Use the below line for setting the project path when running in standalone
    //std::string project_path = "/home/abowery/Desktop/OpenIFS/projects/";
    ...
    std::string var2 = std::string("cp ") + project_path + \
                       std::string("openifs_wu_") + exptid + std::string(".zip ") + slot_path;
    fprintf(stderr, "Copying namelist files to working directory: %s\n", var2.c_str());
    fflush(stderr);
    system(var2.c_str());

    // Copy ancils files to working directory
    std::string var3 = std::string("cp ") + project_path + \
                       std::string("openifs_ic_ancil_") + exptid + std::string(".zip ") + slot_path;
    fprintf(stderr, "Copying ancils files to working directory: %s\n", var3.c_str());
    fflush(stderr);
    system(var3.c_str());

    // Copy ifsdata files to working directory
    std::string var4 = std::string("cp ") + project_path + \
                       std::string("ifsdata_") + exptid + std::string(".zip ") + slot_path;
    fprintf(stderr, "Copying ifsdata files to working directory: %s\n", var4.c_str());
    fflush(stderr);
    system(var4.c_str());
    ...
    std::string var9 = slot_path + std::string("/openifs_app_") + version + std::string(".zip");
    fprintf(stderr, "Unzipping app zip file: %s\n", var9.c_str());
    fflush(stderr);
    boinc_zip(UNZIP_IT,var9.c_str(),slot_path);

    // Unzip the ancils zip file
    std::string var10 = slot_path + std::string("/openifs_wu_") + exptid + std::string(".zip");
    fprintf(stderr, "Unzipping namelist zip file: %s\n", var10.c_str());
    fflush(stderr);
    boinc_zip(UNZIP_IT,var10.c_str(),slot_path);

    // Unzip the ancils zip file
    std::string var11 = slot_path + std::string("/openifs_ic_ancil_") + exptid + std::string(".zip");
    fprintf(stderr, "Unzipping ancils zip file: %s\n", var11.c_str());
    fflush(stderr);
    boinc_zip(UNZIP_IT,var11.c_str(),slot_path);

    // Unzip the ifsdata zip file
    std::string var12 = slot_path + std::string("/ifsdata_") + exptid + std::string(".zip");
    fprintf(stderr, "Unzipping ifsdata files: %s\n", var12.c_str());
    fflush(stderr);
    boinc_zip(UNZIP_IT,var12.c_str(),slot_path);

    // Make the ifsdata directory
    std::string var13a = slot_path + std::string("/openifs_app/ifsdata");
    if (mkdir(var13a.c_str(),S_IRWXU|S_IRWXG|S_IROTH|S_IXOTH) != 0) fprintf(stderr, "mkdir() ifsdata failed\n");

    // Move ifsdata files to app directory
    std::string var13 = std::string("mv ") + slot_path + std::string("/ifsdata_") + exptid + \
                        std::string("/* ") + slot_path + std::string("/openifs_app/ifsdata");
    fprintf(stderr, "Moving ifsdata files to app directory: %s\n", var13.c_str());
    fflush(stderr);
    system(var13.c_str());

    // Move workunit files to app directory
    std::string var14 = std::string("mv ") + slot_path + std::string("/openifs_wu_") + exptid + \
                        std::string("/* ") + slot_path + std::string("/openifs_app");
    fprintf(stderr, "Move workunit files to app directory: %s\n", var14.c_str());
    fflush(stderr);
    system(var14.c_str());

    // Move ancil files to app directory
    std::string var15 = std::string("mv ") + slot_path + std::string("/openifs_ic_ancil_") + exptid + \
                        std::string("/* ") + slot_path + std::string("/openifs_app");
    fprintf(stderr, "Move ancil files to app directory: %s\n",var15.c_str());
    fflush(stderr);
    system(var15.c_str());

    char *pathvar;
    // Set the GRIB_SAMPLES_PATH environmental variable
    std::string var16 = std::string("GRIB_SAMPLES_PATH=") + slot_path + \
                        std::string("/openifs_app/eccodes/ifs_samples/grib1_mlgrib2");
    if (putenv((char *)var16.c_str())) {
    ...
    std::string var17 = std::string("GRIB_DEFINITION_PATH=") + slot_path + \
                        std::string("/openifs_app/eccodes/definitions");
    if (putenv((char *)var17.c_str())) {
      fprintf(stderr, "putenv failed \n");
      return 1;
    }
    ...
    std::string var18 = std::string("chmod -R 777 ") + slot_path + std::string("/openifs_app");
    fprintf(stderr, "Changing permissions: %s\n", var18.c_str());
    fflush(stderr);
    system(var18.c_str());

    // Change directory to the app directory
    std::string var18a = slot_path + std::string("/openifs_app/");
    if (chdir(var18a.c_str()) != 0) fprintf(stderr, "chdir() failed to: %s\n",var18a.c_str());

    // Defaults to input arguments
    int OIFS_RUN=1;                   // run number, output will be saved to directory: output$OIFS_RUN
    int OIFS_RES=159;	              // model resolution
    std::string OIFS_EXPID = exptid;  // model experiment id, must match string in filenames
    ...
    std::string NAMELIST="fort.4";    // NAMELIST file, usually taken from 'ecmwf' subdirectory
    int TIMESTEP=3600;
    std::string FCLEN = "d1";
    std::string FCLEN_TIMESTEP=" -f "+FCLEN+" -t "+std::to_string(TIMESTEP);
    ...
    std::string var19a = std::string("OIFS_DUMMY_ACTION=abort");
    if (putenv((char *)var19a.c_str())) {
      fprintf(stderr, "putenv failed \n");
      return 1;
    }
    ...
    std::string var19b = std::string("OMP_NUM_THREADS=") + std::to_string(NTHREADS);
    if (putenv((char *)var19b.c_str())) {
      fprintf(stderr, "putenv failed \n");
      return 1;
    }
    ...
    std::string var19c = std::string("OMP_SCHEDULE=STATIC");
    if (putenv((char *)var19c.c_str())) {
      fprintf(stderr, "putenv failed \n");
      return 1;
    }
    ...
    std::string var19ca = std::string("DR_HOOK=1");
    if (putenv((char *)var19ca.c_str())) {
      fprintf(stderr, "putenv failed \n");
      return 1;
    }
    ...
    std::string var19cb = std::string("DR_HOOK_HEAPCHECK=no");
    if (putenv((char *)var19cb.c_str())) {
      fprintf(stderr, "putenv failed \n");
      return 1;
    }
    ...
    std::string var19cc = std::string("DR_HOOK_STACKCHECK=no");
    if (putenv((char *)var19cc.c_str())) {
      fprintf(stderr, "putenv failed \n");
      return 1;
    }
    ...
    std::string var19d = std::string("OMP_STACKSIZE=128M");
    if (putenv((char *)var19d.c_str())) {
      fprintf(stderr, "putenv failed \n");
      return 1;
    }
    ...
    if(NAMELIST != "fort.4") {
       ...
       std::string var19e = std::string("mv ") + namelist_file + slot_path + std::string("/openifs_app/fort.4");
    ...
    std::string var19 = std::string("cd ") + slot_path + \
    ...
    std::string var20a = slot_path + std::string("/openifs_result_") + wuid;
    if (mkdir(var20a.c_str(),S_IRWXU|S_IRWXG|S_IROTH|S_IXOTH) != 0) fprintf(stderr, "mkdir() openifs_result failed\n");

    // Move the results to the results folder
    std::string var20 = std::string("mv -t ") + slot_path + std::string("/openifs_result_") + wuid + std::string(" ") + \
    ...
    std::string var22 = std::string("tar cf openifs_result_") + wuid + std::string(".tar openifs_result_") + wuid + \
    ...
    std::string var23 = std::string("cp ") + slot_path + \
    ...
    std::string var24 = project_path + std::string("/openifs_result_") + wuid + std::string(".tar");
    fprintf(stderr, "File being uploaded: %s\n", var24.c_str());
    fflush(stderr);
    boinc_upload_file(var24);
